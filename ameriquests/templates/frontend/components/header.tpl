 ...
 *
 * MODIFIED/CHANGED/UPDATED: Tao You @vanderbilt -- 2020.04
 * Customized header.tpl for AmeriQuest journal at Vanderbilt Library 
...
<meta name="google-site-verification" content="3eCJGFK0swNSgZfDaSJSnnZbUijGtu9me1krbrGyvhc" />
<body class="pkp_page_{$requestedPage|escape|default:"index"} pkp_op_{$requestedOp|escape|default:"index"}{if $showingLogo} has_site_logo{/if}">
	<div class="pkp_structure_page">
		...
		{* Header *}
		<!-- Customized Header for AmeriQuest --> 
		<header class="navbar navbar-default" id="headerNavigationContainer" role="banner">

			<!-- Customized site-name div for AmeriQuest --> 
			{* Logo or site title. Only use <h1> heading on the homepage.
					...
					{/if}
						...
						{if $displayPageHeaderLogo && is_array($displayPageHeaderLogo)}
							<a href="{$homeUrl}">
								<img class="img-responsive img-logo" src="{$publicFilesDir}/{$displayPageHeaderLogo.uploadName|escape:"url"}" {if $displayPageHeaderLogo.altText != ''}alt="{$displayPageHeaderLogo.altText|escape}"{/if}>
							</a>
						{elseif $displayPageHeaderTitle && !$displayPageHeaderLogo && is_string($displayPageHeaderTitle)}
							<a href="{$homeUrl}" class="navbar-brand">{$displayPageHeaderTitle}</a>
						{elseif $displayPageHeaderTitle && !$displayPageHeaderLogo && is_array($displayPageHeaderTitle)}
							<a href="{$homeUrl}">
								<img class="img-responsive img-logo" src="{$publicFilesDir}/{$displayPageHeaderTitle.uploadName|escape:"url"}" alt="{$displayPageHeaderTitle.altText|escape}">
							</a>
						{else}
							<a href="{$homeUrl}">
								<img class="img-responsive img-logo" src="{$baseUrl}/templates/images/structure/logo.png" alt="{$applicationName|escape}" title="{$applicationName|escape}" />
							</a>
						{/if}
					{if $requestedOp == 'index'}
						</h1>
